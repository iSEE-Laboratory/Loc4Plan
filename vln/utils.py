...
def get_config(config_file, oracle):
    ...
    config = DotDict()
    # thl
    config['tokenizer'] = get_value_or_default('tokenizer', 'spm', input_config)
    assert config.tokenizer in ['spm', 'bert']

    config['use_progress_score'] = get_value_or_default('use_progress_score', False, input_config)
    assert config.use_progress_score in [True, False]


    config['use_subInstruction'] = get_value_or_default('use_subInstruction', False, input_config)
    assert config.use_subInstruction in [True, False]


    config['rnn1_step'] = get_value_or_default('rnn1_step', False, input_config)
    assert config.rnn1_step in [True, False]
    config['rnn2_step'] = get_value_or_default('rnn2_step', False, input_config)
    assert config.rnn2_step in [True, False]
...
def conditional_output(str, logger):
    if logger is not None:
        logger.info(str)
    print(str)

def get_file_path(service_id, file_name, opts=None):
    folder = "/root" if service_id == 666 else "/home/huilin"
    if file_name == 'node_instr_pair':
        file_path = "VLN/code/thl_vln/result_visualization/tmp_files/" + f'{opts.dataset}_node_instr_pair_all.pkl'
    else:
        # feature
        file_path = 'VLN/code/map2seq_vln/panorama_preprocessing/fourth_layer/output/touchdown_resnet.pickle'
    return os.path.join(folder, file_path)

def heading_normalization(heading, heading_change_noise, is_test):
    heading_norm_final = []
    for i in range(heading.shape[0]):
        a = heading[i, 0, 0].item()
        if -180 < a <= 180:
            a = a
        elif a > 180:
            a = a - 360
            ...
            a = a + 360
        assert -180 < a <= 180
        heading_change = a / 180
        if heading_change_noise > 0 and not is_test:
            noise = np.random.normal(0.0, abs(heading_change * heading_change_noise))
            if heading_change <= 0 and (heading_change + noise) >= 0:
                noise = 0
            if heading_change >= 0 and (heading_change + noise) <= 0:
                noise = 0
            ...
            heading_norm_final.append(heading_change)
        else:
            heading_norm_final.append(heading_change)
    heading_norm_final = np.asarray(heading_norm_final, dtype=np.float32)
    heading_norm_final = torch.from_numpy(heading_norm_final).to(heading.device)
    return heading_norm_final.unsqueeze(-1).unsqueeze(-1)
