...
class BaseNavigator:
    ...
    def get_action_heading(self, heading_change_noise, is_test):
        go_towards = ['forward', 'left', 'right']
        action_heading = []

        _, current_heading = self.graph_state
        for action in go_towards:
            _, next_heading = self._get_next_graph_state(self.graph_state, action)
            a = next_heading - current_heading
            if -180 < a <= 180:
                a = a
            elif a > 180:
                a = a - 360
            ...
            action_heading.append(heading_change)
        action_heading.append(0)
        return action_heading
