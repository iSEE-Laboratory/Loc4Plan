...
panos_dir = '/root/VLN/data/streetlearn/jpegs_manhattan_touchdown_2022'
...
for i, (panoid, node) in enumerate(sorted(graph.nodes.items())):
    ...
    for heading in headings:
        ...
        features_all[heading] = image_feature
    ...
    all_pano_features[panoid] = features_all


    n_processed += 1
    print(n_processed, 'of', len(graph.nodes))

# with open(os.path.join(output_dir, 'touchdown_resnet_fourth_layer.pickle'), 'wb') as f:
#     pickle.dump(all_pano_heading_features, f)
with open(os.path.join(output_dir, 'touchdown_resnet_fourth_layer_pano.pickle'), 'wb') as f:
        pickle.dump(all_pano_features, f)
